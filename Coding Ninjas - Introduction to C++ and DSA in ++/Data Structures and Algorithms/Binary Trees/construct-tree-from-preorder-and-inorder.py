...
def buildTreePreOrder(preorder, inorder):
    ...
    if preLength!=inLength or preLength == 0 or inLength == 0 or preLength == None or inLength == 0:
        return None
    
    root = BinaryTreeNode(preorder[0])
    ri = inorder.index(root.data)
    
    root.left = buildTreePreOrder(preorder[1:ri+1], inorder[:ri])
    root.right = buildTreePreOrder(preorder[ri+1:], inorder[ri+1:])
    return root
